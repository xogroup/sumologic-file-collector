...
SUMO_CLOBBER=${SUMO_CLOBBER:=true}
SUMO_SOURCE_NAME=${SUMO_SOURCE_NAME:=localfile-collector-container}
SUMO_SOURCE_CATEGORY=${SUMO_SOURCE_CATEGORY:=collector-container}

# TODO
# SUMO_SOURCE_PATH_EXPRESSION=${SUMOLOGIC_SOURCE_PATH_EXPRESSION:="/tmp/clogs/\**"}
...
generate_user_properties_file() {
    ...
    # replace source config params with env variables
    sed -i -e  "s/_SUMO_SOURCE_NAME_/$SUMO_SOURCE_NAME/g" $SUMO_SOURCES_JSON
    sed -i -e  "s/_SUMO_SOURCE_CATEGORY_/$SUMO_SOURCE_CATEGORY/g" $SUMO_SOURCES_JSON

    # TODO
    # sed -i -e  "s/_SUMO_SOURCE_PATH_EXPRESSION_/$SUMOLOGIC_SOURCE_PATH_EXPRESSION/g" $SUMO_SOURCES_JSON
